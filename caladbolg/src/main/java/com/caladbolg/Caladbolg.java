...
public class Caladbolg extends DialogFragment implements OnClickListener,
    ...
    public static Caladbolg newInstance(Fragment fragment, int initialColor, int listElement) {
        Caladbolg caladbolg = new Caladbolg();
        caladbolg.initialize(initialColor);
        caladbolg.setTargetFragment(fragment, 0);
        caladbolg.setListElement(listElement);
        return caladbolg;
    }
    ...
    private void setListElement(int listElement) {
...
//        builder.setNegativeButton(getActivity().getString(R.string.dialog_negative_btn), this);
    ...
    public void onClick(DialogInterface dialog, int which) {
        switch (which) {
            case Dialog.BUTTON_POSITIVE :
                if (mCallback != null) mCallback.onPickColor(mRGB, mAlpha, listElement);
                break;
    ...
    public interface ColorPickerCallback {
        void onPickColor(int rgb, int alpha, int listElement);
        void onCancel();
    }
}
